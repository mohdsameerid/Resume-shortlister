...
export default function NavBar() {
  ...
  const backPath = current > 0 ? STEPS[current - 1].path : null
  ...
  return (
    <header className="bg-white/80 backdrop-blur-sm border-b border-gray-100 px-6 sticky top-0 z-10">
      <div className="max-w-7xl mx-auto flex items-center h-14 gap-6">
        {/* Brand */}
        <span className="text-sm font-bold bg-gradient-to-r from-indigo-600 to-violet-600 bg-clip-text text-transparent tracking-tight shrink-0 select-none">
          ResumeShortlister
        </span>
                    ...
                    <div
                      className={[
                        'w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold transition-all duration-200',
                          ...
                          : active
                          ? 'bg-gradient-to-br from-indigo-500 to-violet-600 text-white ring-4 ring-indigo-100 shadow-sm shadow-indigo-200'
